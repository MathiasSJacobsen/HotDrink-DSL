...
export class HotDrinkDslValidator {
    ...
    checkArgumentOnlyReferenceToVarOnce(
        argument: Arguments,
        ...
        if (argument.ref) {
            ...
            if (s.size !== argument.ref.length) {
                accept(
                    "error",
                    "Can not use the same variable more then once in an argument.",
                    { node: argument, property: "ref" }
            ...
            if (s1.size !== argument.final.length) {
                accept(
                    "error",
                    "Can no use the same variable more then once in an argument.",
